...
export default function StoryCard({ story }: StoryCardProps) {
  ...
  return (
    <Link href={`/chat/${story.id}`}>
      <Card className="group bg-[var(--card-bg)] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl hover-scale transition-all duration-300 cursor-pointer">
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <Badge 
                variant="secondary" 
                className={`bg-gradient-to-r ${getCategoryGradient(story.category)} text-xs border-0`}
              >
                {story.category}
              </Badge>
            </div>
            <h4 className="text-white font-semibold text-sm mb-1">{story.title}</h4>
            <p className="text-gray-300 text-xs line-clamp-2">{story.description}</p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1">
                ...
                <span className="text-gray-400 text-sm">{formatNumber(story.views)}</span>
              </div>
              <div className="flex items-center space-x-1">
                <Heart className="h-4 w-4 text-red-400" />
                <span className="text-gray-400 text-sm">{formatNumber(story.likes)}</span>
              </div>
              <div className="flex items-center space-x-1">
                <Share2 className="h-4 w-4 text-gray-400" />
                <span className="text-gray-400 text-sm">{formatNumber(story.shares)}</span>
              </div>
            </div>
          </div>
